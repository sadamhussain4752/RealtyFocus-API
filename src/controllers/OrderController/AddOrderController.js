...
const sendWhatsAppMessage = async (phoneNumber,verificationCode,Ordervalues) => {
  console.log(phoneNumber,verificationCode,Ordervalues,'phoneNumber,verificationCode,Ordervalues');
...
exports.createOrder = async (req, res) => {
  try {
   ...
   if(offline_type){
    let item_card = `${newProduct.name} - ${newProduct.description}\nPayment Status: ${paymentStatus}\nAmount: ${totalAmount}`
    try {
            
      const messageBody = `Customize Biryani New Order ID: ${newProduct._id}`;
       let messageId =  onMessageSend(newProduct.paymentStatus,messageBody)
      await sendVerificationSMS("9844015563", newProduct._id,messageId);
      // await sendWhatsAppMessage('9844015563',item_card)
...
exports.updateOrderById = async (req, res) => {
  try {
    ...
    if (status) {
      ...
      const messageBody = `Order has been Done ORDER ID: ${orderId.substring(0, 6)}`;
      if (status === "Cancelled") {
      ...
      sendWhatsAppMessage(address.phone,messageBody,status)
